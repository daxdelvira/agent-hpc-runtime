...
# Clean-tree guard — scoped to what actually breaks commit-attribution.
#
# A trial must be attributable to a commit, so MODIFIED OR STAGED TRACKED files
# abort: the code that runs would differ from HEAD and meta.json's git_commit
# would be a lie. That is the real hazard, and it is already documented --
# three trials today ran a predictor different from their recorded commit.
#
# UNTRACKED files are a different matter, and gating on them cost us two holds:
#   - an .nfs<hex> silly-rename artifact (nobody's edit) idled job 11652948
#   - experiments/bench_format_activation.py, a standalone benchmark that no
#     trial imports, written by a concurrent agent, idled it again for 14 min
# With several agents sharing one tree, a whole-tree cleanliness gate turns
# any unrelated scratch file into lost allocation time.
#
# So: untracked files under experiments/ and scripts/ are ALLOWED and merely
# logged (they are standalone; nothing the runner imports). Untracked files
# under runtime/ or workloads/ still ABORT -- those directories are on the
# import path, so a stray module there genuinely can change what executes.
_tracked_dirty=$(git status --porcelain --ignore-submodules=untracked -- experiments workloads runtime 2>/dev/null \
                 | grep -E '^( M|M |MM|A |AM| D|D |R |C )' || true)
if [ -n "$_tracked_dirty" ]; then
  log "ABORT: modified/staged tracked files — commit first (attribution would be wrong)."
  log "$_tracked_dirty"; exit 1
fi
# runtime/tests/ is excluded: the runner imports runtime.predictor, .prefetch,
# .guard, .adapters -- never runtime.tests. A stray test module cannot change
# what a trial executes, and blocking on one is the same false positive again.
_untracked_importable=$(git status --porcelain --ignore-submodules=untracked -- workloads runtime 2>/dev/null \
                        | grep -E '^\?\?' | grep -v '/\.nfs[0-9a-f]\{16,\}' \
                        | grep -v '^?? runtime/tests/' || true)
if [ -n "$_untracked_importable" ]; then
  log "ABORT: untracked files under runtime/ or workloads/ — these are on the import path."
  log "$_untracked_importable"; exit 1
fi
_untracked_ok=$(git status --porcelain --ignore-submodules=untracked -- experiments 2>/dev/null \
                | grep -E '^\?\?' | grep -v '/\.nfs[0-9a-f]\{16,\}' || true)
if [ -n "$_untracked_ok" ]; then
  log "NOTE: untracked standalone files present (allowed, not imported by any trial):"
  log "$_untracked_ok"
fi
