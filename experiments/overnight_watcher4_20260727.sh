#!/bin/bash
# overnight_watcher4_20260727.sh — pool-campaign supervisor (2026-07-27).
# Successor to overnight_watcher3_20260720.sh (whose jobs 11267691/92 died
# 7/23 before the pool campaign could start — namespace teardown race on
# preemption).  Repointed at the 7/27 L40S chain; the campaign script now
# also runs the sleep/wake microbench as a once-per-facet preflight.
set -u
REPO=/storage/project/r-ag117-0/shared/agent_hpc/agent-hpc-runtime
cd "$REPO"
WLOG=$REPO/logs/overnight_watcher4_20260727.log
mkdir -p "$REPO/logs"
...
while true; do
  ...
  if [ "$alive" = 0 ]; then
    log "all tracked jobs gone; watcher4 exiting"
    exit 0
  fi
  sleep 120
done
