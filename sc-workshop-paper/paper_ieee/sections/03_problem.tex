...
whose construction would otherwise be exposed. What they do not supply is distance: agents insert calls to recover from
syntax errors and retry failures, so the trace diverges from the planned
offsets even when the order is right. Plans reach far but locate poorly, local
transitions locate well but do not reach---which is why both are used.
