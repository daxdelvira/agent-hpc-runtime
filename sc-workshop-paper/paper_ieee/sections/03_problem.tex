...
\caption{Agreement with the modal action at each step index, over 137
executions of an identical task.}
...
against a state-of-the-art DMSH system~\cite{megammap} on a workflow with short
inter-step
...
\caption{Retention value per gigabyte. Models occupy a narrow band set by
transfer bandwidth; data artifacts bracket it on both sides, so no fixed class
priority is correct.}
...
\caption{As the resource population grows at fixed schedule length, first
uses become a larger share of needs and the advantage moves from retention to
staging. Against the recency-ranked baseline.}
...
\centering
% PLOT SPEC (kept for provenance; rendered by
% scripts/figures/make_figures.py -> figure_drafts/fig-prediction-signals.pdf)
%   MERGED from the former fig:tool-relationships and fig:plan-accuracy, which
%   backed the two paragraphs of this subsection and cost two captions and two
%   floats. Top: share of AtomAgents tools whose best successor at offset k
%   clears a given confidence, with a star at the best-over-any-k count.
%   Bottom: plan compliance per planned step, order-only against positional,
%   as grouped bars -- the two scorings for a step now sit side by side rather
%   than a column apart, and the positional zeros are labelled because a
%   zero-height bar is otherwise indistinguishable from a missing one.
\includegraphics[width=\columnwidth]{figures/fig-prediction-signals}
\caption{The two horizon signals. Top: 17 of 26 AtomAgents tools have a
successor at some offset above 0.9 confidence. Bottom: plan compliance per
step over 64 runs---scored on order alone the plan holds, scored positionally
it collapses after the first retry, so planner orderings are accurate and
planner-implied distances are not.}
\label{fig:prediction-signals}
\end{figure}


\paragraph{Local transition structure survives workflow-level nondeterminism.}
Agentic workflows are dynamic in the large but structured in the small: one
tool often follows another a fixed number of invocations later. Over the
offsets our transition table records ($k \in \{1,2,3\}$), 17 of 26 AtomAgents
tools have a successor with empirical confidence of at least 0.9, concentrated
at $k=2$ (Figure~\ref{fig:prediction-signals}). These relationships persist
across different prompts and objectives within a framework, so prior traces
supply priors for a workflow never seen before.

\paragraph{Plans predict order, not distance.}
A planner agent exposes a second, longer-range signal: a declared plan naming
the tools to be invoked and their order. Scored on order alone, AtomAgents
plans are 76.3\% compliant over 64 runs, far enough ahead to be actionable.
Scored positionally, so that a retry consumes a plan slot, compliance falls to
40.8\% and every step after the first divergence reads as a miss
(Figure~\ref{fig:prediction-signals}): agents insert recovery and retry calls,
