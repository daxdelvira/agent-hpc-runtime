...
computational chemistry~\cite{ai_hpc_survey}. For instance, agents that selectively prune data to
limit simulation to high-potential pathways (protein configurations, material
models, etc.) lead to significant performance gains, such as in
DeepDriveMD~\cite{deepdrivemd_og,parslfest_ddmd_pres}, where performance
improved by $2.33\times$, or in AI-driven multiscale simulation of viral
protein dynamics~\cite{amaro_covid_data}. Other agentic workflows increase
...
metallic alloy design~\cite{atomagents}.
...
the infamous ``memory wall'' problem~\cite{ai_memwall} results in longer wall time for
...
overheads of storage access. This ``Deep Memory and Storage Hierarchy''
(DMSH)~\cite{hermes,megammap,hades} could be presented to applications as a
massive memory pool without incurring significant performance penalties. As illustrated in
...
that a state-of-the-art DMSH system~\cite{megammap} increases end-to-end wall time by
...
constrained~\cite{nvidia_h200_costs}, and the usual escape---provisioning more
DRAM or bandwidth---is
...
We evaluate \sysname{} on workflows built with the AtomAgents
framework~\cite{atomagents},
...
\item We characterize the failure of deep memory and storage hierarchy (DMSH)
staging under AI steering: a state-of-the-art system increases wall time by
$3.18\times$, and randomizing its staging decisions does not degrade it
further, isolating the absence of a staging window rather than prediction
error as the cause. Byte placement is also the wrong unit of work, since
representation change within the consumer process accounts for 93.0\% of the
cost of making an artifact usable and the value of a retained gigabyte varies
by $65\times$ across representations.

\item We present \sysname, which arbitrates residency for agent-backing models
and transformed scientific data against a single host-memory budget, ranking
candidates by stall avoided per gigabyte held and restricting speculation to
unclaimed memory. It reduces end-to-end wall time by 34.5--50.8\% relative to a
runtime that retains nothing, and by a further 10.3--17.1\% relative to a
