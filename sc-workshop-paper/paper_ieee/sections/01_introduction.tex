...
\item \textbf{The problem, and what it implies.} AI steering breaks the
assumption behind deep memory and storage hierarchy (DMSH) systems: that a
phase's data needs are known before it begins. A state-of-the-art one makes
wall time $3.18\times$ worse on an agentic workflow, and randomising its
predictions does not worsen it---what it lacks is a window, not accuracy. Byte
placement is the wrong unit regardless: 93.0\% of the cost of making an
artifact usable is representation change inside a live process, and a retained
gigabyte varies $65\times$ in value.

\item \textbf{The system, and what it buys.} \sysname{} arbitrates residency
for agent-backing models and transformed scientific data against one
host-memory budget, ranking by stall avoided per gigabyte held and bounding
speculation to memory retention has not claimed. It cuts end-to-end wall time
by 34.5--50.8\% against a runtime that retains nothing and a further
10.3--17.1\% against a recency-ranked baseline that already retains both, with
the margin widening as budget and device capacity grow.
