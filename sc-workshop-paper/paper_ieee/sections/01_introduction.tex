...
\begin{figure}[t]
\centering
\figspec{5.0cm}{\textbf{fig:intro-behavior} --- two stacked timelines. Top:
a traditional HPC workflow, phases with declared inputs, staging bars for
phase $n{+}1$ overlapping computation of phase $n$, no exposed stall. Bottom:
an agentic workflow, where the data need for phase $n{+}1$ becomes visible
only after the agent's decision point, so the staging bar starts late and a
stall band is exposed. Mark the agent decision points explicitly --- the late
arrival of the need is the whole figure.}
...
Because DMSH solutions are not broadly applicable to agentic workflows, the
performance challenges introduced by these workflows translate into economic
and sustainability concerns for scientific computing infrastructures. HPC
systems operate under strict power, memory, and budget constraints, where idle
accelerators and unnecessary data transfers lead to greater operational costs
and higher energy consumption. Simply scaling DRAM capacity or storage
bandwidth to accommodate dynamic data-access patterns is rarely feasible,
especially for research institutions operating fixed on-premise infrastructure
or handling sensitive data that cannot be exposed to externally hosted
services. Efficient data movement therefore becomes a first-order systems
concern rather than a secondary optimization. At the same time, agentic
workflows offer the potential to accelerate scientific discovery through
adaptive experimentation and automated decision-making. Enabling these
workflows to run efficiently within realistic resource constraints therefore
represents a systems challenge with broad scientific impact.
...
each candidate according to the time until it is next required. Relative to a
runtime that retains nothing, \sysname{} reduces end-to-end wall time by 34.5\%
at our production allocation and by 50.8\% with additional accelerator
capacity. Relative to a recency-ranked baseline that already retains both
resource classes, it reduces wall time by a further 10.3--17.1\%. We report
both because retention alone is obtainable from an inference server and a
...
resources only into memory that the arbitrator has left unclaimed. This
restriction makes speculation safe at realistic prediction quality: a stage
that consumes only unclaimed memory cannot displace a more valuable retention,
so its downside is bounded by wasted bandwidth rather than a lost retention.
Displacement is permitted only under a conjunction of strict conditions.
Staging contributes a further 1.6 percentage points at the production
allocation, and its share grows with the number of distinct resources a
workflow touches.

We evaluate \sysname{} in terms of stall reduction and end-to-end wall time,
isolate the contribution of each mechanism, measure the CPU cost that resident
worker processes impose on foreground agent work, and characterize sensitivity
to the planning horizon, accelerator topology, and predictor accuracy. We
compare against a runtime that retains nothing, a recency-ranked retention
policy that already retains both resource classes, and a state-of-the-art DMSH
system evaluated both as naively implemented and as adapted to account for
agentic framework specifics, using workflows built with the AtomAgents
framework.
