...
\sysname{} manages two kinds of resource: the language models backing an
agent's reasoning and tool-calling steps, and the scientific data artifacts
those tools consume. Each sits in one of three states---\emph{active} and
serving, \emph{released} so that the next use pays full construction, or
\emph{held}, in which it is not serving but the expensive part of making it
usable has been paid and is being kept. Held means parked weights for a model,
...
artifact it means a resident worker keeping the parsed structure across tool
invocations.
...
to any file-backed cache and charged to one allocation. \sysname{} therefore
budgets them together rather than tiering them separately---a parked model and
a retained database compete for exactly the same bytes. A retention arbitrator
decides what stays held; a subordinate prefetcher moves resources toward
residency ahead of need. Both score candidates with one value function, so
staging and keeping are directly comparable.
